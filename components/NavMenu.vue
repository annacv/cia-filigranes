...
const menuItemLinkClasses = 'hover:border-b-2 hover:border-opacity-50 border-white pb-1';
const menuItemClasses = 'overflow-hidden text-white transition-[max-height,padding] duration-500 ease-in-out max-h-16';
...
const updateExpandedState = () => {
  routes.forEach((routeItem, index) => {
    if (routeItem.children) {
      expandedItems.value[index] = shouldBeExpanded(routeItem);
    }
  });
};
  ...
  <nav role="navigation">
    <ul>
      ...
      <component
        ...
        :key="index"
        :class="[menuItemClasses, { 'max-h-[28rem] pb-2': isExpanded(index) }]"
      >
        <div class="flex flex-row py-4 gap-8 justify-start items-baseline">
          <NuxtLinkLocale
            :to="routeItem.name"
            :class="[menuItemLinkClasses, { 'border-b-2': isRouteActiveExact(`/${routeItem.name}`) }]"
            @click="emit('toggle')"
          >
          ...
          <ul v-if="routeItem.children && isExpanded(index)">
            ...
            >
              <NuxtLinkLocale
                ...
                :class="[menuItemLinkClasses, { 'border-b-2': isRouteActiveExact(`/${routeItem.name}/${child}`) }]"
                @click="emit('toggle')"
              >
